...
class JCAPPProductLog: NSObject {
    
    //单例
    static let shared = JCAPPProductLog()
    ...
    public func registe(with env: EnvType) {
        ...
        let fileLoggerManagerDefault = JCAPPProductLogFile.init(logsDirectory: logDirectory)
        /// file log
...
extension JCAPPProductLog: DDLogFormatter {
...
extension NSObject {
    public func deallocPrint() {
        JCAPPProductLog.debug("deinit: \(NSStringFromClass(type(of: self)))")
    }
}
