//
//  JCAPPDeviceAuthorizationTool.m
//  JoyCash
//
...
@interface JCAPPDeviceAuthorizationTool ()<CLLocationManagerDelegate>

@property (nonatomic, strong) CLLocationManager *locationManager;

@end

@implementation JCAPPDeviceAuthorizationTool

+ (instancetype)authorization {
    static JCAPPDeviceAuthorizationTool *au;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (au == nil) {
            au = [[JCAPPDeviceAuthorizationTool alloc] init];
        }
    });
...
+ (NSArray<NSString *> *)deviceLocationInfo {
    return @[[NSString stringWithFormat:@"%f", [JCAPPDeviceAuthorizationTool authorization].location.coordinate.latitude],
             [NSString stringWithFormat:@"%f", [JCAPPDeviceAuthorizationTool authorization].location.coordinate.longitude]];
}
...
- (void)geocoderInfoForLocation:(CLLocation *)location {
    if (!JCAPPDeviceNetObserver.shared.netReachable) {
        return;
    }
